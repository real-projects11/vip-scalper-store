...
export default function Admin() {
  ...
  async function load() {
    try {
      const [statsRes, pendingRes, salesRes, productsRes] = await Promise.all([
  ...
  async function crearProducto() {
    ...
    try {
      ...
      if (!res.ok) { setMsg({ ok: false, text: data.error }); }
        ...
        setNewName('');
        load();
        window.location.href = `/admin/edit/${data.product.slug}`;
      }
    } catch {
      setMsg({ ok: false, text: 'No se pudo conectar con el servidor' });
    }
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '32px 20px 60px', fontFamily: '-apple-system, Inter, sans-serif', color: '#111' }}>
      <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 20 }}>Panel</h1>

      <div style={{ marginBottom: 28, border: '1px solid #eee', borderRadius: 12, padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <strong style={{ fontSize: 14 }}>Productos ({products.length})</strong>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
          ...
          <button
            ...
            style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: '#111', color: '#fff', fontWeight: 700, fontSize: 12.5, cursor: 'pointer' }}
          >
            + Nuevo producto
          </button>
        </div>
        ...
        {products.map((p) => (
          <div key={p.slug} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderTop: '1px solid #f2f2f2' }}>
            <div>
              ...
              <div style={{ fontSize: 11.5, color: p.active ? '#146B52' : '#999' }}>
                {p.active ? 'Activo' : 'Borrador'} · ${p.price} · {p.sold} vendidos
              </div>
            </div>
            <a href={`/admin/edit/${p.slug}`} style={{ fontSize: 12.5, fontWeight: 700, color: '#12664F', textDecoration: 'none' }}>Editar →</a>
          </div>
        ))}
      </div>
      ...
      {stats && (
        <>
          ...
          {msg && <p style={{ fontSize: 12.5, margin: '0 0 24px', color: msg.ok ? '#146B52' : '#B34A25' }}>{msg.text}</p>}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
            {pending.map((p) => (
              <div key={p.reservationId} style={{ background: '#fff', border: `1.5px solid ${p.buyerConfirmedAt ? '#E0B04D' : '#E2E4DE'}`, borderRadius: 14, padding: '14px 16px' }}>
                ...
                {p.productTitle && <div style={{ fontSize: 12.5, fontWeight: 600, marginTop: 4 }}>{p.productTitle}</div>}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {sales.map((s) => (
              <div key={s.reservationId} style={{ border: '1.5px solid #eee', borderRadius: 12, padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14 }}>${s.price.toFixed(2)}{s.productTitle ? ` · ${s.productTitle}` : ''}</div>
