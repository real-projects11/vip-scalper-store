...
import { getProduct, incrementSold } from './products';
...
export async function createReservation(contact, productSlug) {
...
async function finalizePaid(key, reservationId, r, txId) {
  ...
  if (r.productSlug) await incrementSold(r.productSlug);
  await redis.zrem(PENDING_IDX, reservationId);
...
export async function getPendingReservations() {
  ...
  // primero los que ya avisaron "ya pagué"
