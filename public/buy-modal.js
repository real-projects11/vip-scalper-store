...
// con el checkout: QR, wallet, timer, polling, y al final el botón de descarga.
...
(function () {
  ...
  function renderCheckout() {
    ...
    overlay.innerHTML = sheet(`
      ...
      <div style="font-size:11.5px;color:#888;margin:10px 0 16px;line-height:1.5;">
        Enviá el monto EXACTO, con los 6 decimales.<br>Es lo que identifica tu compra.
      </div>
      ...
      <button id="copy-wallet-btn" style="width:100%;padding:11px;border-radius:10px;border:1px solid #333;background:#161616;color:#fff;font-size:12.5px;font-weight:600;cursor:pointer;margin-bottom:18px;">
        📋 Copiar wallet
      </button>

      <div style="display:flex;align-items:center;justify-content:center;gap:8px;font-size:12.5px;color:#999;">
    ...
    pollInterval = setInterval(async () => {
      try {
        ...
        if (result.status === 'paid') {
          ...
          clearInterval(timerInterval);
          const overlay = document.getElementById('buy-modal-overlay');
          overlay.innerHTML = sheet('<p style="color:#e46;">El tiempo para pagar venció. Cerrá y volvé a intentar.</p>');
          bindClose();
        }
      } catch (err) {
